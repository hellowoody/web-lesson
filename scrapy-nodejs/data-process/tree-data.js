...
// 把 like 字符串（如 "5.6万" "1198" "547"）转成真实数值
...
function bucketLike(num) {
    ...
    if (num < 100000000) {
        const leadingDigit = Math.floor(num / 10000)
        return `${leadingDigit}万+`
    }
    const leadingDigit = Math.floor(num / 100000000)
    return `${leadingDigit}亿+`
}

const d1 = deduplication_arr.reduce((acc, item) => {
    const level = bucketLike(parseLikeToNumber(item.like))
